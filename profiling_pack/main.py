...
for dataset_name, df in df_dict.items():
    ...
    profile = ProfileReport(df, title=f"Profiling Report for {dataset_name}")
    ...
    for col in pack.df_source.columns:
        non_null_count = max(
            pack.df_source[col].notnull().sum(), 0
        )  # Ensure non-negative
    ...
    p_cells_missing_value = pack.metrics.data[
        pack.metrics.data["key"] == "p_cells_missing"
    ]["value"].values[0]
